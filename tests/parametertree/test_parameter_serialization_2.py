import qtpy.QtGui

from pyqtgraph.parametertree import Parameter
from qtpy.QtGui import QColor
from qtpy import QtCore
from collections import OrderedDict

import pyqtgraph as pg

import pymodaq_gui.utils.widgets.table as table

import pytest

from serializall.factory import SerializableFactory

ser_factory = SerializableFactory()

def test_simple_param_serialization_deserialization():
    param = Parameter.create(
        name="test",
        type="group",
        children=[
            dict(name="ch1", type="bool", value=True),
            dict(name="ch2", type="bool", value=False),
        ],
    )

    serialized_param = ser_factory.get_apply_serializer(param)
    assert isinstance(serialized_param, bytes)
    param_back = ser_factory.get_apply_deserializer(serialized_param)
    assert isinstance(param_back, Parameter)

    assert  pg.eq(param.saveState(), param_back.saveState())


def test_checklist_param_serialization_deserialization():

    params = [{'default': ['a', 'b', 'c'], 'enabled': True, 'exclusive': False, 'expanded': True, 'limits': ['a', 'b', 'c'],
         'name': 'limits', 'readonly': False, 'removable': False, 'renamable': False, 'strictNaming': False,
         'syncExpanded': False, 'title': None, 'type': 'checklist', 'value': ['a', 'b', 'c'], 'visible': True}]

    p = Parameter.create(name='params', type='group', children=params)
    serialized_p = ser_factory.get_apply_serializer(p)
    assert isinstance(serialized_p, bytes)
    p_back = ser_factory.get_apply_deserializer(serialized_p)
    assert isinstance(p_back, Parameter)

    assert  pg.eq(p.saveState(), p_back.saveState())

def test_color_param_serialization_deserialization():

    params = [
              {'default': '#fff', 'enabled': True, 'expanded': True, 'name': 'color', 'readonly': False,
               'removable': False, 'renamable': False, 'strictNaming': False, 'syncExpanded': False, 'title': None,
               'type': 'color', 'value': QColor(255, 0, 0, 255), 'visible': True}
    ]

    p = Parameter.create(name='params', type='group', children=params)
    serialized_p = ser_factory.get_apply_serializer(p)
    assert isinstance(serialized_p, bytes)
    p_back = ser_factory.get_apply_deserializer(serialized_p)
    assert isinstance(p_back, Parameter)

    assert  pg.eq(p.saveState(), p_back.saveState())

##################################################################
# END OK
##################################################################


def test_group_param_serialization_deserialization():
    params = [
        {'title': 'Groups:', 'name': 'groups', 'type': 'group', 'children': [
            {'title': 'A visible group:', 'name': 'agroup', 'type': 'group', 'children': []},
            {'title': 'An hidden group:', 'name': 'bgroup', 'type': 'group', 'children': [], 'visible': False},
            {'title': 'A bool with children:', 'name': 'booleans_group', 'type': 'bool', 'value': False,
             'tip': 'Any Parameter can have its own children', 'children': [
                {'title': 'A bool in a bool', 'name': 'a_bool_in_a_bool', 'type': 'bool', 'value': True},
                {'title': 'A push with children', 'name': 'aboolpush', 'type': 'bool_push', 'value': True,
                 'label': 'action', 'children': [
                    {'title': 'A string in a group', 'name': 'atte_in_a_group', 'type': 'str',
                     'value': 'this is a string you can edit'},
                ]}, ]},
        ]}
    ]

    p = Parameter.create(name='params', type='group', children=params)
    serialized_p = ser_factory.get_apply_serializer(p)
    assert isinstance(serialized_p, bytes)
    p_back = ser_factory.get_apply_deserializer(serialized_p)
    assert isinstance(p_back, Parameter)

    assert pg.eq(p.saveState(), p_back.saveState())


def test_bool_param_serialization_deserialization():
    params = [
        {'title': 'Booleans:', 'name': 'booleans', 'type': 'group', 'children': [
            {'title': 'Standard bool', 'name': 'abool', 'type': 'bool', 'value': True},
            {'title': 'bool push', 'name': 'aboolpush', 'type': 'bool_push', 'value': True, 'label': 'action'},
            {'title': 'A led', 'name': 'aled', 'type': 'led', 'value': False, 'tip': 'a led you cannot toggle'},
            {'title': 'A led', 'name': 'anotherled', 'type': 'led_push', 'value': True, 'tip': 'a led you can toggle'},
        ]}
    ]

    p = Parameter.create(name='params', type='group', children=params)
    serialized_p = ser_factory.get_apply_serializer(p)
    assert isinstance(serialized_p, bytes)
    p_back = ser_factory.get_apply_deserializer(serialized_p)
    assert isinstance(p_back, Parameter)

    assert pg.eq(p.saveState(), p_back.saveState())

def test_bool2_param_serialization_deserialization():
    params = {'title': 'Standard bool', 'name': 'abool', 'type': 'bool', 'value': True}

    p = Parameter.create(**params)
    serialized_p = ser_factory.get_apply_serializer(p)
    assert isinstance(serialized_p, bytes)
    p_back = ser_factory.get_apply_deserializer(serialized_p)
    assert isinstance(p_back, Parameter)

    assert pg.eq(p.saveState(), p_back.saveState())


def test_numbers_param_serialization_deserialization():
    params = [
        {'title': 'Numbers:', 'name': 'numbers', 'type': 'group', 'children': [
            {'title': 'Standard int', 'name': 'anint', 'type': 'int', 'value': 2},
            {'title': 'Standard float', 'name': 'afloat', 'type': 'float', 'value': 20., 'min': 1.,
             'tip': 'displays this text as a tooltip'},
            {'title': 'Another float', 'name': 'anotherfloat', 'type': 'float', 'value': 123., 'min': 1.,
             'tip': 'displays this text as a tooltip'},
            {'title': 'Linear Slide float', 'name': 'linearslidefloat', 'type': 'slide',
             'value': 50, 'default': 50, 'min': 0, 'max': 123, 'subtype': 'linear'},
            {'title': 'Linear Slide float w limits', 'name': 'linearslidefloatlimits',
             'type': 'slide', 'value': 50, 'default': 50, 'limits': (24, 123), 'subtype': 'linear'},
            {'title': 'Linear int Slide', 'name': 'linearslideint', 'type': 'slide', 'value': 50, 'default': 50,
             'step': 1,
             'min': 0,
             'max': 123, 'subtype': 'linear', 'int': True},
            {'title': 'Linear Slide with suffix', 'name': 'linearslidewithsuffixandsiPrefix', 'type': 'slide',
             'value': 50, 'default': 50,
             'min': 0,
             'max': 1e6, 'subtype': 'linear', 'suffix': 'V', 'siPrefix': True},
            {'title': 'Log Slide float', 'name': 'logslidefloat', 'type': 'slide', 'value': 50, 'default': 50,
             'min': 1e-5,
             'max': 1e5, 'subtype': 'log', 'suffix': 'V', 'siPrefix': True},
        ]}
    ]

    p = Parameter.create(name='params', type='group', children=params)
    serialized_p = ser_factory.get_apply_serializer(p)
    assert isinstance(serialized_p, bytes)
    p_back = ser_factory.get_apply_deserializer(serialized_p)
    assert isinstance(p_back, Parameter)

    assert pg.eq(p.saveState(), p_back.saveState())

def test_date_time_param_serialization_deserialization():
    params = [
        {'title': 'DateTime:', 'name': 'datetimes', 'type': 'group', 'children': [
            {'title': 'Time:', 'name': 'atime', 'type': 'time', 'value': QtCore.QTime.currentTime()},
            {'title': 'Date:', 'name': 'adate', 'type': 'date', 'value': QtCore.QDate.currentDate(),
             'format': 'dd/MM/yyyy'},
            {'title': 'DateTime:', 'name': 'adatetime', 'type': 'date_time',
             'value': QtCore.QDateTime(QtCore.QDate.currentDate(), QtCore.QTime.currentTime()),
             'format': 'MM/dd/yyyy hh:mm', 'tip': 'displays this text as a tooltip'},
        ]}
    ]

    p = Parameter.create(name='params', type='group', children=params)
    serialized_p = ser_factory.get_apply_serializer(p)
    assert isinstance(serialized_p, bytes)
    p_back = ser_factory.get_apply_deserializer(serialized_p)
    assert isinstance(p_back, Parameter)

    assert p == p_back

def test_action_param_serialization_deserialization():
    params = [
        {'title': 'An action', 'name': 'action', 'type': 'action'},
    ]

    p = Parameter.create(name='params', type='group', children=params)
    serialized_p = ser_factory.get_apply_serializer(p)
    assert isinstance(serialized_p, bytes)
    p_back = ser_factory.get_apply_deserializer(serialized_p)
    assert isinstance(p_back, Parameter)

    assert p == p_back


def test_browsepath_param_serialization_deserialization():
    params = [
        {'title': 'Browsing files:', 'name': 'browser', 'type': 'group', 'children': [
            {'title': 'Look for a file:', 'name': 'afile', 'type': 'browsepath', 'value': '', 'filetype': True,
             'tip': 'If filetype is True select a file otherwise a directory'},
            {'title': 'Look for a dir:', 'name': 'adir', 'type': 'browsepath', 'value': '', 'filetype': False,
             'tip': 'If filetype is True select a file otherwise a directory'},

        ]}
    ]

    p = Parameter.create(name='params', type='group', children=params)
    serialized_p = ser_factory.get_apply_serializer(p)
    assert isinstance(serialized_p, bytes)
    p_back = ser_factory.get_apply_deserializer(serialized_p)
    assert isinstance(p_back, Parameter)

    assert p == p_back


def test_itemselect_param_serialization_deserialization():
    params = [
        {'title': 'Selectable items:', 'name': 'itemss', 'type': 'group', 'children': [
            {'title': 'Selectable items', 'name': 'items', 'type': 'itemselect',
             'value': dict(all_items=['item1', 'item2', 'item3', 'item4', 'item5'],
                           selected=['item2']),
             'tip': 'Press Ctrl+click  to select items in any order'},
            {'title': 'Selectable items', 'name': 'itemsbis', 'type': 'itemselect',
             'value': dict(all_items=['item1', 'item2', 'item3'], selected=['item2']),
             'tip': 'If show_pb is True, user can add items to the list', 'show_pb': True, },
            {'title': 'Removable items', 'name': 'itemsbisbis', 'type': 'itemselect',
             'value': dict(all_items=['item1', 'item2', 'item3'], selected=['item2']),
             'tip': 'If show_mb is True, user can remove selected items from the list', 'show_mb': True, },
            {'title': 'Checkable items', 'name': 'itemscheckable', 'type': 'itemselect',
             'value': dict(all_items=['item1', 'item2', 'item3'], selected=['item2']),
             'tip': 'If checkbox is True, user can select item by checking/unchecking items. Remove items is still used with standard selections',
             'show_pb': True, 'checkbox': True, 'show_mb': True, },
            {'title': 'Dragable items', 'name': 'itemsdragablecheckable', 'type': 'itemselect',
             'value': dict(all_items=['item1', 'item2', 'item3'], selected=['item2']),
             'tip': 'If dragdrop is True, user can drag or drop items inside the list', 'checkbox': True,
             'dragdrop': True},
        ]}
    ]

    p = Parameter.create(name='params', type='group', children=params)
    serialized_p = ser_factory.get_apply_serializer(p)
    assert isinstance(serialized_p, bytes)
    p_back = ser_factory.get_apply_deserializer(serialized_p)
    assert isinstance(p_back, Parameter)

    assert p == p_back

def test_text_param_serialization_deserialization():
    params = [
        {'title': 'Plain text:', 'name': 'texts', 'type': 'group', 'children': [
            {'title': 'Standard str', 'name': 'atte', 'type': 'str', 'value': 'this is a string you can edit'},
            {'title': 'Plain text', 'name': 'text', 'type': 'text', 'value': 'this is some text'},
            {'title': 'Plain text', 'name': 'textpb', 'type': 'text_pb', 'value': 'this is some text',
             'tip': 'If text_pb type is used, user can add text to the parameter'},
        ]}
    ]

    p = Parameter.create(name='params', type='group', children=params)
    serialized_p = ser_factory.get_apply_serializer(p)
    assert isinstance(serialized_p, bytes)
    p_back = ser_factory.get_apply_deserializer(serialized_p)
    assert isinstance(p_back, Parameter)

    assert p == p_back

def test_table_param_serialization_deserialization():
    params = [
        {'title': 'Tables:', 'name': 'tables', 'type': 'group', 'children': [
            {'title': 'Table widget', 'name': 'tablewidget', 'type': 'table', 'value':
                OrderedDict(key1='data1', key2=24), 'header': ['keys', 'limits'], 'height': 100},
            {'title': 'Table view', 'name': 'tabular_table', 'type': 'table_view',
             'delegate': table.SpinBoxDelegate, 'menu': True,
             'value': table.TableModel([[0.1, 0.2, 0.3]], ['value1', 'value2', 'value3']),
             'tip': 'The advantage of the Table model lies in its modularity.\n For concrete examples see the'
                    'TableModelTabular and the TableModelSequential custom models in the'
                    ' pymodaq.utils.scanner module'},
        ]}
    ]

    p = Parameter.create(name='params', type='group', children=params)
    serialized_p = ser_factory.get_apply_serializer(p)
    assert isinstance(serialized_p, bytes)
    p_back = ser_factory.get_apply_deserializer(serialized_p)
    assert isinstance(p_back, Parameter)

    assert p == p_back

# TODO
# calendar cmaplut colormap file font pen progress pixmap pixmap_check slider
# slide => slider ?
# check int


def test_list_param_serialization_deserialization():

    params = [
              {'enabled': True, 'expanded': True, 'limits': ['a', 'b', 'c'], 'name': 'widget', 'readonly': False,
               'removable': False, 'renamable': False, 'strictNaming': False, 'syncExpanded': False, 'title': None,
               'type': 'list', 'visible': True}
    ]

    p = Parameter.create(name='params', type='group', children=params)
    serialized_p = ser_factory.get_apply_serializer(p)
    assert isinstance(serialized_p, bytes)
    p_back = ser_factory.get_apply_deserializer(serialized_p)
    assert isinstance(p_back, Parameter)

    assert p == p_back

def test_file_param_serialization_deserialization():

    params = [
              {'acceptMode': None, 'dialogLabel': None, 'directory': None, 'enabled': True, 'expanded': True,
               'fileMode': None, 'name': 'widget', 'nameFilter': None, 'readonly': True, 'relativeTo': None,
               'removable': False, 'renamable': False, 'strictNaming': False, 'syncExpanded': False, 'title': None,
               'type': 'file', 'viewMode': None, 'visible': True, 'windowTitle': None}
    ]

    p = Parameter.create(name='params', type='group', children=params)
    serialized_p = ser_factory.get_apply_serializer(p)
    assert isinstance(serialized_p, bytes)
    p_back = ser_factory.get_apply_deserializer(serialized_p)
    assert isinstance(p_back, Parameter)

    assert p == p_back